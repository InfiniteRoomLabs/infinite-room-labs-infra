...
### Changed
- **Public-readiness scrub (R16): operator-personal data externalized from the repo.** The Paperless classification taxonomy (correspondents/document types/tags/workflows name the operator's actual institutions) moved to gitignored `paperless_config_private.yml` with a committed `.example` showing the shape -- group_vars auto-loading means zero playbook changes. `knowledge-backup.sh` now reads private archive paths from local `~/.config/knowledge-backup/dirs` instead of hardcoding them; the laptop DR runbook references that file generically. A personal workflow-design doc was retired from the tree, and incidental personal wording in comments/CHANGELOG was neutralized. Git history rewritten to match (git-filter-repo); stale pre-rewrite branches deleted on both remotes; the GitHub copy was recreated fresh (old PRs pinned pre-rewrite objects). Public-launch furniture added: MIT LICENSE, SECURITY.md, README license section, and a GitHub Actions `hygiene` workflow running the repo-contract suite on every push/PR. Repo flipped PUBLIC on GitHub 2026-08-07; R16 status updated (conventions/verifiability workstreams remain open).
...
### Added
- **R16 showcase polish: ADRs, CI-with-teeth, README header, Renovate.** Five Nygard ADRs in `docs/decisions/` (mise front door, Traefik-over-Caddy, CNPG-over-Bitnami, Bitwarden single-source secrets, NodePort LAN game pattern), each claim traced to repo evidence. Three new GitHub Actions lanes: kubeconform (11 charts schema-validated against k8s 1.31.4), conftest (`policy/*.rego` -- pinned-image/resource/privilege policies, 1075 assertions passing against rendered charts + deploy overlays), trivy config scan (SARIF to the Security tab, non-blocking by design). `renovate.json` watches chart pins in `helm-deploy.yml` (custom manager), image digests, mise pins, and Actions. All `uses:` steps digest-pinned (Renovate `helpers:pinGitHubActionDigests` maintains them). README gains the showcase header: badges, hardware + stack tables, Mermaid topology. Built by a 21-agent gated workflow; quality gates caught a fresh-checkout chart-dependency bug and a CI-vs-ADR contradiction pre-merge.
- **Palworld dedicated server deployed, LAN-reachable** (new `irl-palworld` chart v0.1.0), mirroring the Satisfactory NodePort LAN pattern. Single Deployment (`thijsvanloef/palworld-server-docker` v2.7.1, pinned) in `irl`, deployed via `helm-deploy.yml --tags palworld` (Phase 5, `wait: false` -- first boot is a multi-Gi SteamCMD pull behind a 30-min startupProbe). Storage: ZFS dataset `main/palworld-data` (30G, chowned 1000:1000, sanoid `service_data`) -> `pv-palworld-data` -> `palworld-data-pvc`. Exposure: NodePort 30211/udp only (game protocol is UDP-only; container listens on the NodePort number directly); nftables UDP allowlist + `allow-palworld-game-lan` NetworkPolicy admit 192.168.2.0/24, tailnet rides `allow-ingress-tailscale`. Palworld specifics vs Satisfactory: RCON enabled pod-internally for graceful SIGTERM saves -- first game-server secret (`palworld-admin-password` BW item -> `palworld-secrets`/`ADMIN_PASSWORD` via bw-sync); REST API TCP 8212 as probe target (UDP game port is unprobeable); daily 5 AM ET auto-reboot mitigates the well-known server memory leak (16Gi limit as backstop); image-native daily tar backups on top of sanoid snapshots. Registry entry `cluster_only`, runbook `ansible/docs/runbooks/palworld-down.md`, DNS smoke record, backup-and-restore SOP section, access-guide entry.
- **helm-charts submodule bump: `irl-jobops` v0.3.0 per-preset pipeline CronJobs.** `pipelineCron.presets` renders one CronJob per JobOps saved-search preset; adds `daily-midlevel-sustainable` (7:45 AM ET) and `daily-qa-automation` (8:30 AM ET) alongside `daily-agentic-ai` (7:00 AM ET). Offsets are wider than a typical run (~10 min) because the orchestrator 409-rejects rather than queues overlapping runs. Same `jobops-cron-auth` secret for all three; search configs live in JobOps itself.
- **Satisfactory dedicated server deployed, LAN-reachable** (new `irl-satisfactory` chart v0.1.0, helm-charts v0.13.0). Single Deployment (`wolveix/satisfactory-server` v1.9.10, pinned) in `irl`, deployed via `helm-deploy.yml --tags satisfactory` (Phase 5, `wait: false` -- first boot is a ~10Gi SteamCMD pull behind a 30-min startupProbe). Storage: new ZFS dataset `main/satisfactory-config` (40G, chowned 1000:1000 in `zfs.yml` since fsGroup ignores hostPath PVs, sanoid `service_data` for hourly save snapshots) -> `pv-satisfactory-config` -> `satisfactory-data-pvc` (explicit `volumeName`). Exposure: NodePort Service with game 30777 (first mixed TCP+UDP Service in the stack) + reliable-messaging 30888/tcp; the container listens on the NodePort numbers directly (`SERVERGAMEPORT`/`SERVERMESSAGINGPORT`) because Satisfactory 1.1+ advertises the messaging port to clients. Deliberately LAN-exposed via nftables allowlist additions (30777 tcp+udp, 30888 tcp) so the Tailscale-less Steam Deck connects at `192.168.2.2:30777`; tailnet clients ride the unconditional `tailscale0` accept. Registry entry is `cluster_only` (game traffic, no Traefik route, no homepage tile); no secrets (admin password set in-game at claim time, lives on the PVC). New runbook `ansible/docs/runbooks/satisfactory-down.md`; DNS smoke record in `tests/test_dns.py`.
- **helm-charts submodule bump: `irl-jobops` v0.2.0 daily pipeline CronJob.** Runs the `daily-agentic-ai` JobOps saved search at 7:00 AM ET via `POST /api/pipeline/run` `{"presetName":...}` (fork feature) through the public hostname -- the chart deliberately has no in-cluster route. Auth secret `jobops-cron-auth` created out-of-band; search config lives in JobOps itself, editable in its UI without touching the chart.
- **Public-showcase research salvaged + tracked as R16.** The 2026-07-23 prior-art survey (admired Flux-homelab repo conventions: onedr0p/bjw-s/cluster-template + kubesearch.dev; clig.dev CLI doctrine; Diataxis/MADR docs scaffolding; kubeconform->SLSA verifiability ladder) had been delivered agent-to-agent and never written to disk -- now recovered from the session transcript into `docs/plans/resources/2026-07-23-public-showcase-conventions.md`, with new RESEARCH.md item R16 (public-readiness of this repo) linking it to the agent-ops `public-readiness` skill / `oss-health-checker` agent for the eventual leak scrub. Also folds in the pending RESEARCH.md hygiene edits (R02 Gitea-vs-GitLab closed as "migrate to Forgejo" with the 2026-07-21 migration plan doc now committed, encoding normalized to ASCII).
